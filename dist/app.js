...
function render(){
 ...
 $('detail').innerHTML=`<div class="detail-body"><span class="tag">${selected===current[day]?'いま ここ':s.kind}</span><h2>${s.kana}</h2><p class="kanji">${s.name}</p><p class="description">${s.description}</p><div class="actions"><button id="listen">なまえを きく ♪</button><button id="write" class="primary">ひらがなを かく</button></div><div class="actions" id="placeActions"><button id="arrive" class="arrive">${done?'✓ ついた！ を とりけす':'ついた！'}</button>${selected!==current[day]?'<button id="setCurrent">ここから すすめる</button>':''}</div><p id="speechStatus" role="status"></p></div><div class="next-card"><small>${next?'つぎは':'きょうの ゴール'}</small><p><strong>${next?next.kana:trip[day].stay}</strong></p>${next?`<p>${s.ride}</p><button id="goNext">つぎへ すすむ →</button>`:day<2?'<button id="nextDay">つぎの ひを みる →</button>':''}</div>`;
 showDestinationPhoto(s);
 const placeQuery=mapPlace(s);if(placeQuery){const a=document.createElement('a');a.className='map-button';a.href='https://www.google.com/maps/search/?'+new URLSearchParams({api:'1',query:placeQuery});a.target='_blank';a.rel='noopener noreferrer';a.textContent='Google マップ ↗';a.setAttribute('aria-label',s.name+'を Google マップで ひらく');$('placeActions').append(a);}
